...
| 平台 | 包与状态 |
| --- | --- |
| Android 8.0+ 手机 | 源码已更新到 `0.2.50+56`；两版三架构 APK 由 GitHub Actions 构建，使用固定发布证书签名并在构建中校验证书指纹，可从 Release 直接下载；真实安装与运行仍待验收 |
...
| Android TV | 与手机共用 Android 源码；已补强自动识别与电视模式横屏，待电视 / 盒子实机验收；0.2.12 选集 / 设置及 0.2.13 同步仍待集中验证 |
| iOS 15.1+ | 已加入工程、Go 核心链接、媒体依赖、文件管理、系统代理桥接、Bonjour / 局域网权限和构建脚本；0.2.40 起 iOS 播放页禁用 media_kit_video 硬件纹理加速以规避 libmpv OpenGL ES 渲染上下文失败直接退出；Actions 已产出未签名 IPA，需用自签工具安装，尚无已签名 IPA 和真机验收 |
